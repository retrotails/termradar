...
res_img = [7000, 3500] # Map resolution
...
except: exit("Could not parse config file, \"" + path.join(conf_dir, "config") + "\", try deleting it.")
...
if len(col_actual) != 16:
	exit("\"termcolors\" configuration option needs 16 values (has " + str(len(col_actual)) + "). Try deleting " + conf_dir)
...
res = [0,0] # Size of scaled/cropped view
res_scale = 0 # Scale to terminal
offset = [0,0] # Crop offset
res_term = [get_terminal_size().columns, get_terminal_size().lines]

# Check terminal size
if args.size:
	s = split('x',args.size)
	if (is_int(s[0]) and is_int(s[1])): res_term = [int(s[0]), int(s[1]) ]
	else: exit("Error: Invalid size")
if args.all: rect = [0, 0, res_img[0], res_img[1]]
res_term[1] = res_term[1] * 2
if (res_term[0] < 16 or res_term[1] < 16): exit("Terminal window too small (<16x16)")
...
def main():
	if args.update:
		...
		exit("Error: No images, please use --update")
